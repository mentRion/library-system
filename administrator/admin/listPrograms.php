<?php
if (Input::exists()) {
		$PNG_TEMP_DIR =  dirname(__FILE__).DIRECTORY_SEPARATOR.'uploads'.DIRECTORY_SEPARATOR.'qaFiles'.DIRECTORY_SEPARATOR;
		
		$program = DB:: getInstance()->get('programs', array('id','=',Input::get('id')));							
		foreach($program->results() as $program){
			unlink($PNG_TEMP_DIR.$program->attachedFile);
		}
		
		$programs = DB:: getInstance()->delete('programs', array('id','=',Input::get('id')));													
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Programs Offered</li>
                    </ol>
                </section>
                        ...
                        <div class="col-xs-12">
                            <div class="box box-primary">
                                ...
                                <div class="box-body table-responsive">
                                    ...
                                    <?php }?>
									<?php if(Session::exists('ProgramAdded')) { ?>
                                                 <div class="alert alert-success">
                                                    <i class="glyphicon glyphicon-ok"></i> &nbsp;<?php echo Session::flash('ProgramAdded'); ?>
                                                 </div>
                                        <?php }?>
                                    <table class="table table-bordered table-hover" id="articles">
                                        <thead>
                                            <tr>
												<th>Graduate Studies / Undergraduate</th>
                                        ...
                                        <tbody>
											<?php
												...
												foreach($programs->results() as $programs){
													$programType = DB:: getInstance()->query("SELECT * FROM programtype WHERE id = '$programs->programType'");							
													foreach($programType->results() as $programType){	?>
														<tr>
														
															<td><?php echo $programType->type ; ?></td>
															<td><?php echo $programs->programs ; ?></td>
															<td><?php echo $programs->accLevel.' '.$programs->accPhase; ?></td>
															<td>
																<a href="admin/uploads/qaFiles/<?php echo $programs->attachedFile; ?>" target="_blank">
																	<?php echo $programs->attachedFile ; ?>
																</a>
															</td>
															<td align="center">
																<?php require_once ('delete-confirm.php');?>
																<form method="POST" action="" style="display:inline">
																	<input type="hidden" name="id" value="<?php echo $programs->id;  ?>">
																...
																<form method="POST" action="admin.php?action=editProgram&&id=<?php echo $programs->id; ?>" style="display:inline">
								...
								<div class="box-footer">
									...
									<div class="modal-dialog" role="document">
										...
										<div class="modal-header">
											...
											<h3 class="modal-title" id="exampleModalLabel">Add New Program</h3>
										</div>
										<div class="modal-body">
											<form enctype="multipart/form-data" method="post" action="addProgram.php">
												<label class="control-label" for="programType"><font color="#EC0003">*</font> Type of Program:</label>
												<div class="form-group">
													<select class="form-control" name="programType" id="programType" required>
														<option hidden value="">Select Program Type</option>
														<?php
															$programType = DB:: getInstance()->query("SELECT * FROM programtype");							
															foreach($programType->results() as $programType){
														?>
														<option value="<?php echo $programType->id?>"><?php echo ucwords($programType->type) ?></option>
														<?php }?>
													</select> 
												</div>
												<label class="control-label" for="program"><font color="#EC0003">*</font> Academic Program</label>
												<div class="form-group">
													<input type="text" class="form-control" id="program" name="program" placeholder="Input Program" required>
												</div>
												<label class="control-label" for="accLevel">Level of Accreditation</label>
												<div class="form-group">
													<input type="text" class="form-control" id="accLevel" name="accLevel" placeholder="Input Level">
												</div>
												<label class="control-label" for="accPhase">Phase of Accreditation</label>
												<div class="form-group">
													<input type="text" class="form-control" id="accPhase" name="accPhase" placeholder="Input Level">
												</div>
												<label class="control-label" for="pppFile">PPP File</label>
												<div class="form-group">
													<input type="text" class="form-control" id="pppFile" name="pppFile" placeholder="Input PPP Name">
												</div>
